...
public static class ConfigLoader
{
    public static Config GetConfig(string xml)
    {
        ...
        var c = new Config
        {
            Namespace = config.DocumentElement.Attributes["namespace"].Value
        };
        ...
        foreach (XmlElement child in config.DocumentElement.ChildNodes)
        {
            ...
            foreach (XmlElement prop in child.ChildNodes)
            {
                ...
                p.IsOptional = prop.HasAttribute("optional") && prop.Attributes["optional"].Value == "true";
                p.IsList = prop.HasAttribute("list") && prop.Attributes["list"].Value == "true";
            }
        }
        return c;
    }
...
public class Node
{
    ...
    private readonly List<NodeProperty> _props = new();
    ...
    public bool IsTopMost => Config.Nodes.All(x => x.BasedOn != Name);
