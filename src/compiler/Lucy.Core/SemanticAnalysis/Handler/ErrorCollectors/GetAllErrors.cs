...
{
    [DbQuery] ///<see cref="GetAllErrorsEx.GetAllErrors"/>
    ...
    private static ComparableReadOnlyList<ErrorWithRange> Remap(IDb db, ComparableReadOnlyList<Error> errors)
    {
        ...
        foreach(var error in errors)
        {
            result.Add(error switch 
            {
                ...
                ErrorWithNodeId ewni => new ErrorWithRange(ewni.NodeId.DocumentPath, db.GetRangeFromNodeId(ewni.NodeId), ewni.Message),
